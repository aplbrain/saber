...
from conduit.utils.job_definitions import (
...
class CwlParser:
    def __init__(self, cwl, config, constant=False, optimization_iteration=None):
        ...
        self.job_param_def = self.dj_hook.create_definition(
            self.cwl["inputs"], self.workflow_name
        )
    ...
    def create_job_definitions(self):
        for stepname, tool in self.steps.items():
            ...
            if self.local:
                ...
                self.job_def_arns[stepname] = create_job_definition(job_def)[
                    "jobDefinitionArn"
                ]
    ...
    def resolve_tools(self):
        ...
        tool_yamls = {}
        toollist = [
            (step_name, os.path.normpath(os.path.join(self.cwl_fp, step["run"])))
    ...
    def create_subdag(
        self, iteration, i, param_db_update_dict, job_params, job, wf_id, deps, dag=None
    ):
        subdag_id = "{}_{}.{}".format(self.workflow_name, wf_id, i)
        parent_dag_id = "{}_{}".format(self.workflow_name, wf_id)
        if dag == None:
            subdag = DAG(
                default_args=self.default_args, dag_id=subdag_id, schedule_interval=None
            )
        else:
            subdag = dag
        subdag_steps = {}
        for stepname, tool in self.steps.items():
            ...
            (in_string, out_string) = generate_io_strings(
                tool, wf_id, iteration_parameters, i
            )
            if self.local:
                iteration_parameters["_saber_home"] = wf_id
                iteration_parameters["_saber_stepname"] = "{}/{}".format(
                    wf_id, stepname_c
                )
            else:
                iteration_parameters["_saber_home"] = "{}/{}".format(
                    job["_saber_bucket"], wf_id
                )
                iteration_parameters["_saber_stepname"] = "{}:{}/{}".format(
                    job["_saber_bucket"], wf_id, stepname_c
                )
            if in_string:
            ...
            try:
                score_format = self.cwl["steps"][stepname]["hints"]["saber"][
                    "score_format"
                ]
            except KeyError:
                score_format = ""
            ...
            try:
                ...
                if not self.local:
                    file_path = "{}:{}".format(
                        job["_saber_bucket"], os.path.join(file_path, stepname_c)
                    )
            except KeyError:
                file_path = None
            ...
            if is_local:
                if not self.local:
                    ...
                    t = SaberDockerOperator(
                        ...
                        command=" ".join(
                            sub_params(command_list, iteration_parameters)
                        ),
                        dag=subdag,
                        pool="Local",
                    )
                ...
                t = AWSBatchOperator(
                    ...
                    job_name=re.sub(
                        "[^A-Za-z0-9-_]+",
                        "",
                        "{}-{}".format(self.workflow_name, stepname_c),
                    ),
                    job_definition=self.job_def_arns[stepname],
                    overrides={"command": command_list},
        ...
        self.dj_hook.update(
            param_db_update_dict, classdef=self.job_param_def, primary_keys=unique_keys
        )
        for dep in deps:
    ...
    def generate_dag(self, job, **kwargs):
        ...
        try:
            ...
            log.warning(
                "Workflow database entry for {} already exists, reinserting".format(
                    self.workflow_name
                )
            )
            pass
        ...
        dag = DAG(dag_id=dag_id, default_args=self.default_args, schedule_interval=None)
        job_params, deps = self.resolve_args(job)
        if len(self.parameterization) > 1:
            log.info(
                "Parameterization produces {} workflows, totaling {} jobs...".format(
        ...
        for param in self.cwl["inputs"]:
            if type(job[param]) != dict:
                ...
                raise ValueError(
                    "Unable to insert parameter {} into job parameter database".format(
                        param
                    )
                )
        try:
            ...
            if param in job:
                ...
                if type(job[param]) != dict:
                    ...
                    raise NotImplementedError(
                        "Job parameter value type {} not supported yet".format(
                            job[param]["class"]
                        )
                    )
                ...
                if "?" in self.cwl["inputs"][param]:
                    ...
                    raise TypeError(
                        "Parameter {} is required by workflow, but not present in job description".format(
                            param
                        )
                    )
            ...
            for input_name, inp in step["in"].items():
                if type(inp) == dict and "default" in inp:
                    ...
                    if type(output_deps[inp]) == dict and "default" in output_deps[inp]:
                        step_params[step_name][input_name] = "{}/{}".format(
                            inp.split("/")[0], output_deps[inp]["default"]
                        )
                    else:
                        step_params[step_name][input_name] = "{}/{}".format(
                            inp.split("/")[0], inputs[output_deps[inp]]
                        )
        ...
        for tool_name, tool_info in self.steps.items():
            ...
            if not len(tool_info["outputs"]) == 0:
                for output_name, output in tool_info["outputs"].items():
                    ...
                    resolved_outputs[
                        "{}/{}".format(tool_name, output_name)
                    ] = self.resolve_glob(
                        tool_name=tool_name, glob=output["outputBinding"]["glob"]
                    )
    ...
    def dag_write(self, dag):
        ...
        with open(
            os.path.join(dag_folder, "{}_dag.pickle".format(self.workflow_name)), "wb"
        ) as fp:
            pickle.dump(dag, fp)
        with open(
            os.path.join(dag_folder, "{}_dag.py".format(self.workflow_name)), "w"
        ) as fp:
