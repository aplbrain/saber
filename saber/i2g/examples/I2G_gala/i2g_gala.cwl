...
#Deployment pipeline for dense segmentation
...
inputs:
    ...
    data: File
    train_file: File #trained classifier
    mode: int
    seed_thres: int?
    agg_thres: float?
    height: int?
...
steps:
    membrane_detection:
        run: ../tools/membrane_detection.cwl
        in:
            input: data
            height : height
            width : width
    ...
    synapse_detection:
        run: ../tools/synapse_detection.cwl
        in:
            input: data
            height: height
            width: width
    ...
    neuron_segmentation:
        run: ../tools/neuron_segmentation.cwl
        in:
            mode: mode
