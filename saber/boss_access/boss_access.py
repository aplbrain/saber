...
def boss_merge_xbrain(args):
    ...
    x_block_pad = [np.amax([args.xmin-args.padding,x_min_bound]), np.amin([args.xmax+args.padding,x_max_bound])]
    y_block_pad = [np.amax([args.ymin-args.padding,y_min_bound]), np.amin([args.ymax+args.padding,y_max_bound])]
    z_block_pad = [np.amax([args.zmin-args.padding,z_min_bound]), np.amin([args.zmax+args.padding,z_max_bound])]

    #Coordinates of core data block in local coordinate frame
    xstart = np.amin([args.padding,args.xmin-x_min_bound])
    xend = np.amax([data.shape[0]-args.padding,data.shape[0]-(x_max_bound-args.xmax)])
    ystart = np.amin([args.padding,args.ymin-y_min_bound])
    yend = np.amax([data.shape[1]-args.padding,data.shape[1]-(y_max_bound-args.ymax)])
    zstart = np.amin([args.padding,args.zmin-z_min_bound])
    zend = np.amax([data.shape[2]-args.padding,data.shape[2]-(z_max_bound-args.zmax)])
    ...
    if args.onesided: 
        ...
        for ind in bad_inds:
            xi = np.array([centroids[ind,0]-np.ceil(templatesize/2),centroids[ind,0]+np.ceil(templatesize/2)]).astype(int)
            yi = np.array([centroids[ind,1]-np.ceil(templatesize/2),centroids[ind,1]+np.ceil(templatesize/2)]).astype(int)
            zi = np.array([centroids[ind,2]-np.ceil(templatesize/2),centroids[ind,2]+np.ceil(templatesize/2)]).astype(int)
            data[xi,yi,zi] = 0
        ...
        #Test side 4
        if xend<data.shape[0]: #There is padding on side 4 of cube [xmax+pad:xmax+2*pad,pad:ymax+2*pad,pad:zmax+2*pad]
            margin = pull_margin_cutout(chan_actual,[x_block[1],x_block_pad[1]],[y_block[0],y_block_pad[1]],[z_block[0],z_block_pad[1]])
            # print(data.shape)
            # print(xstart,xend,ystart,yend,zstart,zend)
            # print(x_block, y_block,z_block)
            # print(x_block_pad,y_block_pad,z_block_pad)
            # print(margin.shape)
            # print(data[xend:data.shape[0],ystart:data.shape[1],zstart:data.shape[1]].shape)
            data[xend:data.shape[0],ystart:data.shape[1],zstart:data.shape[2]] = np.maximum(data[xend:data.shape[0],ystart:data.shape[1],zstart:data.shape[2]], margin) 
        #Test side 5
        if yend<data.shape[1] :#There is padding on side 5 of cube [pad:xmax+2*pad,ymax+pad:ymax+2*pad,pad:zmax+2*pad]
            margin = pull_margin_cutout(chan_actual,[x_block[0],x_block_pad[1]],[y_block[1],y_block_pad[1]],[z_block[0],z_block_pad[1]])
            data[xstart:data.shape[0],yend:data.shape[1],zstart:data.shape[2]] = np.maximum(data[xstart:data.shape[0],yend:data.shape[1],zstart:data.shape[2]], margin)
        #Test side 6
        if zend<data.shape[2]: #There is padding on side 4 of cube [pad:xmax+2*pad,pad:ymax+2*pad,zmax+pad:zmax+2*pad]
            margin = pull_margin_cutout(chan_actual,[x_block[0],x_block_pad[1]],[y_block[0],y_block_pad[1]],[z_block[1],z_block_pad[1]])
            data[xstart:data.shape[0],ystart:data.shape[1],zend:data.shape[2]] = np.maximum(data[xstart:data.shape[0],ystart:data.shape[1],zend:data.shape[2]], margin)

        #push results over entire padded area 
        #Pipeline Data will be in X,Y,Z format
        #Change to Z,Y,X for upload
        data = data[xstart:data.shape[0],ystart:data.shape[1],zstart:data.shape[2]]
        data = np.transpose(data,(2,1,0))
        data = data.copy(order='C').astype(eval('np.{}'.format(args.dtype)))
        # Verify that the cutout uploaded correctly.
        rmt.create_cutout(chan_actual, 0, [x_block[0],x_block_pad[1]], [y_block[0],y_block_pad[1]], [z_block[0],z_block_pad[1]], data)
        # Clean up.
    else:
        #Binarize Map
        data[np.where(data>0)]=1
        ...
        bad_inds = []
        for i in range(0,n_centers):
            if centroids[i,0] + templatesize/2 < xstart or centroids[i,0] - templatesize/2 > xend:
                bad_inds.append(i)
            elif centroids[i,1] + templatesize/2 < ystart or centroids[i,1] - templatesize/2 > yend:
                bad_inds.append(i)
            elif centroids[i,2] + templatesize/2 < zstart or centroids[i,2] - templatesize/2 > zend:
                bad_inds.append(i)
        centroids_out = np.delete(centroids,bad_inds, axis=0)
        #translate into global coordinates from local data block
        centroids_out[:,0] = centroids_out[:,0] - xstart + args.xmin
        ...
        #Pull down existing boundary area, if area is valid
        #Test side 1
        if xstart>0: #There is padding on side 1 of cube [0:pad,0:ymax+2*pad,0:zmax+2*pad]
            margin = pull_margin_cutout(chan_actual,[x_block_pad[0],x_block[0]],y_block_pad,z_block_pad)
            data[0:xstart,:,:] = np.maximum(data[0:xstart,:,:],margin)
        #Test side 2
        if ystart>0: #There is padding on side 2 of cube [pad:xmax+2*pad,0:pad,pad:zmax+2*pad]
            margin = pull_margin_cutout(chan_actual,[x_block[0],x_block_pad[1]],[y_block_pad[0],y_block[0]],[z_block[0],z_block_pad[1]])
            data[xstart:data.shape[0],0:ystart,zstart:data.shape[2]] = np.maximum(data[xstart:data.shape[0],0:ystart,zstart:data.shape[1]],margin)
        #Test side 3
        if zstart>0: #There is padding on side 3 of cube [pad:xmax+2*pad,pad:ymax+2*pad,0:pad]
            margin = pull_margin_cutout(chan_actual,[x_block[0],x_block_pad[1]],[y_block[0],y_block_pad[1]],[z_block_pad[0],z_block[0]])
            data[xstart:data.shape[0],ystart:data.shape[1],0:zstart] = np.maximum(data[xstart:data.shape[0],ystart:data.shape[1],0:zstart],margin)
        #Test side 4
        if xend<data.shape[0]: #There is padding on side 4 of cube [xmax+pad:xmax+2*pad,pad:ymax+2*pad,pad:zmax+2*pad]
            margin = pull_margin_cutout(chan_actual,[x_block[1],x_block_pad[1]],[y_block[0],y_block_pad[1]],[z_block[0],z_block_pad[1]])
            data[xend:data.shape[0],ystart:data.shape[1],zstart:data.shape[2]] = np.maximum(data[xend:data.shape[0],ystart:data.shape[1],zstart:data.shape[2]], margin) 
...
def main():
    ...
    parent_parser.add_argument('--host', required=False, default= 'api.bossdb.org', help='Name of boss host: api.bossdb.org')
    ...
    parent_parser.add_argument('--onesided', type=int, default=0, help='flag for one-sided padding') #indicates whether padding is one-sided or two-sided

    push_parser = subparsers.add_parser(
        'push', help='Push images to boss', parents=[parent_parser]
    )
    pull_parser = subparsers.add_parser(
        'pull', help='Pull images from boss and optionally save to AWS S3',
        parents=[parent_parser]
    )
    merge_parser = subparsers.add_parser(
        'merge', help='Merge xbrain images to boss',
        parents=[parent_parser]
    )
    ...
    merge_parser.add_argument('-o', '--output', required=True, help='Output centroid file')
